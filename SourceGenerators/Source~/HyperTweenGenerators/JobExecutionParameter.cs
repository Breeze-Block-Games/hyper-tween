...
public class JobExecutionParameter(IParameterSymbol parameterSymbol, ITypeSymbol invokeComponentSymbol)
{
    ...
    private static Type GetType(IParameterSymbol parameterSymbol, ITypeSymbol invokeComponentSymbol)
    {
        ...
        if (fullName == "BreezeBlockGames.HyperTween.UnityShared.API.TweenFactory")
        {
            ...
            {
                (RefKind.None,"BreezeBlockGames.HyperTween.UnityShared.TweenBuilders.EntityCommandBufferTweenBuilder") => Type.TweenFactory,
                (RefKind.None,"BreezeBlockGames.HyperTween.UnityShared.TweenBuilders.EntityCommandBufferParallelWriterTweenBuilder")  => Type.ParallelTweenFactory,
                _ => throw new InvalidOperationException($"No RefKind expected for TweenFactory and TweenFactory type must be EntityCommandBufferTweenBuilder or EntityCommandBufferParallelWriterTweenBuilder. RefKind: {parameterSymbol.RefKind}, fullName: {typeArgumentSymbols.First().GetFullName()}")
            };
        }
    ...
    public string GetUsing(int index)
    {
        ...
        {
            return "using BreezeBlockGames.HyperTween.UnityShared.ECS.Update.Components;";
        }
